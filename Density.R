...
diamonds_small <-as.vector(things[1441,])
diamonds_big <- as.vector(controles[1441,])
pdf('density_1440_e-9_e-11.pdf')
d <- density(diamonds_small)
plot(d, main="Concentration density at time 1440 with kon e-9 koff e-11")
...
dat <- data.frame(molecules=c(retro.hist,control.hist),lines=rep(c("retro","control"),each=93))


#Plot.
pdf('density_1440_e-9_e-11_V2.pdf')
ggplot(dat, aes(x = molecules, fill = lines)) + geom_density(alpha = 0.5) + opts(title = expression("Concentration density at time 1440 with kon e-9 koff e-11"))
dev.off()
